...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	// エンジンの初期化
	KamataEngine::Initialize(L"LE3D_05_イタイ_ユウキ_CG5");
	...
	ID3D12Resource* depthStencilTextureResource = CreateDepthStencilTextureResource(device, WinApp::kWindowWidth, WinApp::kWindowHeight);
	...
	// DSVHeapの先頭に DSVを作る
	device->CreateDepthStencilView(depthStencilTextureResource, &dsvDesc, dsvHandleCPU);

	//===================================================================================
	// SRV(Shader Resource View)を準備する　※　PixelShaderと連携を取るようにするため

	//-----------------------------------------------------------------------
	// 1. SRV用の DescriptorHeapを作成
	...
	srvDesc.Texture2D.MipLevels = 1;                                            // MipLevelsは 1つしかない
	...
	while (true) {
		...
		// World変換行列の定数バッファの転送
		worldTransform.rotation_.y += 0.005f; // 適当な角度（ラジアン）
